...
echo -e "${CYAN}[ADJUST][BUILD]:${GREEN} Moving shared schemas to generate dynamic ODM Plugin framework using Carthage ... ${NC}"
mv Adjust.xcodeproj/xcshareddata/xcschemes/AdjustSdkWebBridge.xcscheme .
mv AdjustOdmPlugin.xcscheme Adjust.xcodeproj/xcshareddata/xcschemes
echo -e "${CYAN}[ADJUST][BUILD]:${GREEN} Done! ${NC}"
...
then
  echo -e "${CYAN}[ADJUST][BUILD]:${GREEN} Building dynamic ODM plugin target with Carthage ... ${NC}"
  #carthage build --no-skip-current
  arch -x86_64 /bin/bash ./scripts/carthage_xcode.sh build --no-skip-current
  echo -e "${CYAN}[ADJUST][BUILD]:${GREEN} Done! ${NC}"
  ...
  echo -e "${CYAN}[ADJUST][BUILD]:${GREEN} Move Carthage generated dynamic ODM Plugin framework to destination folder ... ${NC}"
  cd "Carthage/Build"
  mv "iOS" "${XCF_FRM_ZIP_NAME__ODM_DYNAMIC}"
  zip -r -X "${XCF_FRM_ZIP_NAME__ODM_DYNAMIC}-"${SDK_VERSION}".framework.zip" "${XCF_FRM_ZIP_NAME__ODM_DYNAMIC}"
  cd -
  mv "Carthage/Build/${XCF_FRM_ZIP_NAME__ODM_DYNAMIC}-"${SDK_VERSION}".framework.zip" "${XCF_OUTPUT_FOLDER}/${XCF_OUTPUT_DYNAMIC_FRMK_FOLDER}"
  mv "Carthage/Build/${XCF_FRM_ZIP_NAME__ODM_DYNAMIC}" "${XCF_OUTPUT_FOLDER}/${XCF_OUTPUT_DYNAMIC_FRMK_FOLDER}"
  echo -e "${CYAN}[ADJUST][BUILD]:${GREEN} Done! ${NC}"
else
