...
+ (void)setLogLevel:(AILogLevel)logLevel {
    [[AIAdjustIoFactory getLogger] setLogLevel:logLevel];
}
...
+ (void)setEventBufferingEnabled:(BOOL)enabled {
    if (activityHandler == nil) {
        [[AIAdjustIoFactory getLogger] error:@"Please call `setEventBufferingEnabled` after `appDidLaunch`!"];
        return;
    }

    activityHandler.bufferEvents = enabled;
    if (enabled) [[AIAdjustIoFactory getLogger] info:@"Event buffering is enabled"];
}

+ (void)setMacMd5TrackingEnabled:(BOOL)enabled {
    if (activityHandler == nil) {
        [[AIAdjustIoFactory getLogger] error:@"Please call `setMacMd5TrackingEnabled` after `appDidLaunch`!"];
        return;
    }

    activityHandler.trackMacMd5 = enabled;
    [[AIAdjustIoFactory getLogger] info:@"Tracking of macMd5 is %@", enabled ? @"enabled" : @"disabled"];
}
