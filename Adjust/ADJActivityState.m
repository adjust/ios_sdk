...
- (NSString *)description {
    return [NSString stringWithFormat:@"ec:%d sc:%d ssc:%d ask:%d sl:%.1f ts:%.1f la:%.1f dt:%@ fgt:%d",
            self.eventCount, self.sessionCount, self.subsessionCount, self.askingAttribution, self.sessionLength,
            self.timeSpent, self.lastActivity, self.deviceToken, self.isForgotten];
}
...
- (id)initWithCoder:(NSCoder *)decoder {
    ...
    if ([decoder containsValueForKey:@"isForgotten"]) {
        self.isForgotten = [decoder decodeBoolForKey:@"isForgotten"];
    } else {
        self.isForgotten = NO;
    }
...
- (void)encodeWithCoder:(NSCoder *)encoder {
    ...
    [encoder encodeBool:self.isForgotten       forKey:@"isForgotten"];
...
- (id)copyWithZone:(NSZone *)zone {
    ...
    if (copy) {
        ...
        copy.isForgotten        = self.isForgotten;
